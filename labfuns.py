...
def plotBoundary(classifier, dataset='iris', split=0.7):
    ...
    for (color, c) in zip(colormap, classes):
        ...
        plt.scatter(xTr[trClIdx,0],xTr[trClIdx,1],marker='o',c=np.array([color]),s=40,alpha=0.5, label="Class "+str(c)+" Train")
        plt.scatter(xTe[teClIdx,0],xTe[teClIdx,1],marker='*',c=np.array([color]),s=50,alpha=0.8, label="Class "+str(c)+" Test")
